...
//            a.writeJakobi();  //Jacobi法
//            a.threadAdapter(); //高速化したJacobi法
//            a.printMahalanobis(); //マハラノビス距離計算
            a.printbayes(); //Bayes認識関数
        } catch (Exception e) {
    ...
    public void printbayes() {
        ...
        for (int moji = 0; moji < KadaiData.FILE_NUM; moji++) {
            correct = 0;
            //未知データ
            for (int m = 180; m < 180 + MICHI_DATA; m++) {
...
//                        }
//                        d *= alfa;
//                        buf = yakobi[ch][0].getLambda();
//                        for (int j = 1; j < KadaiData.DATA_SIZE-16; j++) {
////                            System.out.println(yakobi[ch][j].getLambda());
//                            buf +=  yakobi[ch][j].getLambda();
//                        }
////                        System.out.println("\n\n\n");
//                        if (buf != 0) {
//                            buf = Math.log(buf);
